...
print "$$lang_vars{host_db_ID_message}:</td><td $align1><input name=\"host_id_db\" type=\"text\" size=\"10\" maxlength=\"10\">";
...
for ( my $k = 0; $k < scalar(@cc_values); $k++ ) {
	if ( $cc_values[$k]->[0] eq "CM" ) {
		print "<tr><td $align>CM</td><td $align1> <select name=\"cc_id_$cc_values[$k]->[1]\" size=\"1\">";
