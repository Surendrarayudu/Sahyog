...
my $auth_proto=$daten{'auth_proto'} || "";
my $auth_pass=$daten{'auth_pass'} || "";
#$auth_is_key=$daten{'auth_is_key'} || "";
my $auth_is_key="";
my $priv_proto=$daten{'priv_proto'} || "";
...
if ( ! $node ) {
	$gip->print_error_with_head(title=>"$$lang_vars{gestioip_message}",headline=>"$$lang_vars{live_data_message}",notification=>"$$lang_vars{formato_malo_message} (0)",vars_file=>"$vars_file",client_id=>"$client_id") if $error_message;
}
...
if ( $snmp_version == "3" ) {
	...
	$gip->print_error("$client_id","$$lang_vars{formato_malo_message} (1)") if ! $sec_level;
...
my %if_values=();
my ($ifDescr,$ifIndex,$ifType,$ifSpeed,$ifPhysAddress,$ifAdminStatus,$ifOperStatus,$ifName,$ifAlias,$ifInErrors,$ifOutErrors,$ifLastChange,$duplex,$neighbor_ip,$neighbor_port,$duplex_admin,$vlan_num,$vlan_default,$vlan_default_name);
$ifDescr=$ifIndex=$ifType=$ifSpeed=$ifPhysAddress=$ifAdminStatus=$ifOperStatus=$ifName=$ifAlias=$ifInErrors=$ifOutErrors=$ifLastChange=$duplex=$neighbor_ip=$neighbor_port=$duplex_admin=$vlan_num=$vlan_default=$vlan_default_name="";
...
if ( %if_values ) {
	...
	foreach my $key ( sort { $a <=> $b } keys (%if_values) ) {
		...
		if ( $ifSpeed eq "1073741824" ) {
			# for Microsoft
			$ifSpeed="1Gbps";
		} elsif ( $ifSpeed !~ /$na_gray/ ) {
