...
if ( ! $daten{'hostname'} && ! $daten{'host_id_db'} && ! $daten{'host_descr'} && ! $daten{'comentario'} && ! $daten{'ip'} && ! $daten{'loc'} && ! $daten{'cat'} && ! $daten{'int_admin'} && ! $daten{'client_independent'} && ! $cc_value_exists ) {
...
if ( $gip_query ) {
	...
	if ( $anz_values_hosts > 0 ) {
		print "SEARCH_DATA: IP, hostname, comment, location, category, description, AI";
		for ( my $k = 0; $k < scalar(@cc_values); $k++ ) {
			print ", $cc_values[$k]->[0]";
		}
		print "\n\n";
	...
	foreach my $keys ( keys %{$host_hash_ref} ) {
		...
		print "SEARCH_DATA: $query_ip, $query_hostname, $query_comment, $query_loc, $query_cat, $query_description, $query_ai";
