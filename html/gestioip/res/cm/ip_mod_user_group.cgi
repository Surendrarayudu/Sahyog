...
$gip->print_error_with_head(title=>"$$lang_vars{gestioip_message}",headline=>"$$lang_vars{device_user_group_message}",notification=>"$error_message",vars_file=>"$vars_file",client_id=>"$client_id") if $error_message;


$gip->CheckInput("$client_id",\%daten,"$$lang_vars{mal_signo_error_message}","$$lang_vars{device_user_group_modified_message}: $device_user_group_name","$vars_file");
...
while ( my ($key, @value) = each(%values_device_user_groups) ) {
	$gip->print_error("$client_id","$$lang_vars{device_user_group_name_exists_message}") if $value[0]->[0] eq $device_user_group_name && $key ne $id;
}

# change user group 

$gip->mod_device_user_group("$client_id","$id","$device_user_group_name","$device_user_name","$login_pass","$priv_pass","$description");
