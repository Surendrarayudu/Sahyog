...
##### TEST: CM
#		my $cm_enabled=$global_config[0]->[8] || "";
#		if ( $cm_enabled eq "yes" ) {
#			my $config_backup_dir=$global_config[0]->[9] || "";
#			if ( ! $config_backup_dir ) {
#				$gip->print_error("$client_id","$$lang_vars{no_cm_config_dir_message}");
#			}
#		}
#		
#		rename("$config_backup_dir/$client_name_db","$config_backup_dir/$client_name") or $gip->print_error("$client_id","$$lang_vars{can_not_rename_config_dir_message}: $!");
#
...
function confirmation() {
	...
	answer = confirm(CLIENT + ":\\n\\n $$lang_vars{associated_networks_message}: $anz_red_client \\n $$lang_vars{associated_hosts_message}: $anz_host_client\\n\\n$$lang_vars{delete_client_confirme_message}?")
