...
foreach (@mass_update_types) {
	if ( $_ eq $$lang_vars{description_message} ) {
		print "<tr><td><b>$$lang_vars{description_message}</b></td><td><input type=\"text\" size=\"25\" name=\"descr\" value=\"\" maxlength=\"100\">\n";
	}
	if ( $_ eq $$lang_vars{loc_message} ) {
		print "<tr><td><b>$$lang_vars{loc_message}</b></td><td><select name=\"loc\" size=\"1\">\n";
		print "<option></option>"; 
	...
	if ( $_ eq $$lang_vars{cat_message} ) {
		print "<tr><td><b>$$lang_vars{cat_message}</b></td><td><select name=\"cat_net\" size=\"1\">\n";
		print "<option></option>"; 
	...
	if ( $_ eq $$lang_vars{comentario_message} ) {
		print "<tr><td><b>$$lang_vars{comentario_message}</b></td><td><textarea name=\"comentario\" cols=\"30\" rows=\"5\" wrap=\"physical\" maxlength=\"500\"></textarea>";
		print "</td></tr>";
	}
	if ( $_ eq $$lang_vars{sinc_message} ) {
		print "<tr><td><b>$$lang_vars{sinc_message}</b></td><td><input type=\"checkbox\" name=\"vigilada\" value=\"y\"></td></tr>\n";
	}
}
		...
		if ( $cc_name eq $mass_element ) {
                        ...
                        if ( $cc_name eq "vlan" ) {
                                ...
                                if ( ! $vlans[0] ) {
                                        ...
                                        print "<tr><td><b>$cc_name</b></td><td><select name=\"${cc_name}_value\" size=\"5\">";
                                        print "<option></option>";
                                        foreach ( @vlans ) {
				...
				my $fav_checked="";
				print "<tr><td><b>$cc_name</b></td><td><input type=\"checkbox\" name=\"${cc_name}_value\" value=\"yes\">";
			} else {
				print "<tr><td><b>$cc_name</b></td><td><input type=\"text\" size=\"20\" name=\"${cc_name}_value\" value=\"\" maxlength=\"500\">\n";
			}
			print "<input name=\"cc_name\" type=\"hidden\" value=\"$cc_name\"><input name=\"custom_${n}_id\" type=\"hidden\" value=\"$cc_id\"></td></tr>\n";
		}
		$n++;
	}
}
